...
export const getUserJuiceDelta = async (
  ...
  if (!isAllTime) {
    ...
    await Promise.all(
      Object.values(unstakesByToken).map(async (events) => {
        if (!events) return;
        ...
        await Promise.all(
          events.map(async (event) => {
            ...
            const _token = tokens.find(
              (t) =>
                t.address?.toLocaleLowerCase() === token.toLocaleLowerCase()
            );
            const tokenId = _token?.alias || _token?.id || "";

            const prevStake: ethers.Event | undefined = findPrevStake(
              blockNumber,
              stakesByToken[token]
            );
  ...
  await Promise.all(
    _tokens.map(async (token, idx) => {
      const st = res[idx];
      if (st.juiceStake.isZero()) return;

      const prevStake: ethers.Event | undefined = findPrevStake(
        parsedTo,
        stakesByToken[token.address]
      );
      const initBlockNumber = prevStake?.blockNumber || parsedFrom;
